...
def test_api_cache_020(api_client, auth_headers):
    cache_key = uuid.uuid4().hex
    path = f"{MANIFEST_PATH}?cache_key={cache_key}"

    r1 = api_client.get(
        path,
        headers={**auth_headers, "X-Request-ID": f"cache-020-1-{uuid.uuid4()}"}
    )
    assert r1.status_code == 200

    r2 = api_client.get(
        path,
        headers={**auth_headers, "X-Request-ID": f"cache-020-2-{uuid.uuid4()}"}
    )
    assert r2.status_code == 200

    x_cache = r2.headers.get("X-Cache")
    if x_cache is not None:
        assert x_cache in ("HIT", "MISS", "WAIT+HIT")
